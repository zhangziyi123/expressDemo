<template>
  <div class="vue-html5-editor" :class="{'full-screen':fullScreen}" :style="{'z-index':zIndex}">
    <div class="toolbar" :style="{'z-index':zIndex+1}" ref="toolbar">
      <ul>
        <template v-for="module in modules">
          <li :title="locale[module.i18n]" @click="activeModule(module)"  :key="module.name">
    ...
    <div class="content" ref="content" :style="contentStyle" contenteditable @click="toggleDashboard(dashboard)"></div>
  </div>
</template>

<script>
  import vue5Edit from "./index";
  import RangeHandler from './range/handler';
  import './style.less';
  /**
   * Created by peak on 2017/2/9.
   */
  var editObj = new vue5Edit({
    language: 'zh-cn',
    visibleModules: [
      "textCom",
      "color",
  ...
  export default {
    ...
    created() {
      // modules, locale, defaultShowModuleName
      this.modules = editObj.modules;
      this.locale = editObj.locale;
      this.defaultShowModuleName = editObj.defaultShowModuleName;
